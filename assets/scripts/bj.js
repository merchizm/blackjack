...
function joinGame(place) {
    $('.input-panel').hide();
    let data = {command: 'join', place: place};
    let validateW = function (wallet) {
        let r = /^\d*(\.\d+)?[mkMK]?$/;
        if (!r.test(wallet))
            return 'Incorrect wallet format';
        return '';
    };
    let validateN = function (name) {
        if (name.length < 3)
...
function add_card(card, index, id, split) {
    if (id === -1)
        ...
        setTimeout(function () {
            ...
            card.appendTo('.bj-container');
            let topPos = (y_cards[id - 1] + index * 20);
            //0 + 20
            if (split > 0) topPos += 20;
            let leftPos = x_cards[id - 1];
            //-10 + 20
            if (split != 0) leftPos += 15 * split;
            card.animate({top: topPos + 'px', left: leftPos + 'px'}, 200);
            ani_time -= 300;
            if ($('.card-holder-' + id + '-s' + split).length == 0)
                $('<div class="card-holder card-holder-' + id + '-s' + split + '" style="top:' + topPos + 'px;left:' + leftPos + 'px;"></div>').appendTo('.bj-container');
            else
...
function getState(data) {
    ...
    if (data.state !== 4 || (data.state === game_data.state)) {
        ...
        for (i in data.players) {
            ...
            if (data.players[i].split && $('.p' + data.players[i].pos + '-text-split').length == 0) {
                $('<div class="p' + data.players[i].pos + '-text-split"></div>').appendTo('.bj-container');
    ...
    if ((data.state >= 3 && game_data.state >= 3) || game_data.game_id !== data.game_id) {
        ...
        for (p_i in data.players) {
            if (game_data.players[p_i] && data.players[p_i].split && !game_data.players[p_i].split) {
                ...
                $('<div class="card-holder card-holder-' + data.players[p_i].pos + '-s-1" style="top:' + topPos + 'px;left:' + (leftPos - 15) + 'px;"></div>').appendTo('.bj-container');
                $('<div class="card-holder card-holder-' + data.players[p_i].pos + '-s1" style="top:' + (topPos + 20) + 'px;left:' + (leftPos + 15) + 'px;"></div>').appendTo('.bj-container');
    ...
    if (data.state === 1)
        setTimeout(function () {
            $('.start-panel').show();
            if (data.is_allow_cancel)
                $('.cancel-bet').show(); else $('.cancel-bet').hide();
        }, ani_time);
    else
        $('.start-panel').hide();
