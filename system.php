...
function cashFormat($amount): string
{
    $amount += 0;
    if ($amount < 100 * 1000)
...
class BjModel
{
    private array $suits = array('ch', 'cd', 'cc', 'cs');
    private array $cards = array('ca', 'c2', 'c3', 'c4', 'c5', 'c6', 'c7', 'c8', 'c9', 'c10', 'cj', 'cq', 'ck');

    // minimum bet
    private int $min_bet = 1;
    // maximum bet
    private int $max_bet = 500;

    // time to place bets (in seconds)
    private int $round_time = 15;
    // time to make a decision hit/stand etc. (in seconds)
    private int $turn_time = 15;

    public array $data = [];

    public array $ds = [];

    private mixed $sem = null;
    private string $sem_file = 'bj.lck';
    private bool $is_sem_waiting = false;

    function semWait(): void
    {
    ...
    function load(): void
    {
        if (!file_exists('bj_data.json'))
            $this->createNew();
        else {
            $data = json_decode(file_get_contents('bj_data.json'), true);
            $this->ds = $data['ds'];
            $this->data = $data['data'];
        }
    }
    ...
    function save(): void
    {
        $ds = $this->ds;
        $data = $this->data;
        file_put_contents('bj_data.json', '{"ds":' . json_encode($ds) . ',"data":' . json_encode($data) . '}');
    }

    function getNextUID(): int
    {
        $this->semWait();
        $uid = 0;
        ...
        switch ($this->data['state']) {
            case 0:
                ...
                if (count($this->data['players']) == 5) {
                    $this->data['ltime'] = time() + $this->round_time;//time to begin
                    $this->data['state'] = 1;
                    $this->save();
                }
            ...
            case 1:
                ...
                if ($unsets) {
                    if ($this->ds['g_players_current'] == 0) $this->data['state'] = 0;
                    $this->save();

                }
                if ($this->ds['g_players_current'] == 0) {
                    $this->data['state'] = 0;
                    $this->save();
                }
    ...
    function insuranceComplete()
    {
        ...
        $this->data['state'] = 3;
        $this->data['ltime'] = time() + $this->turn_time;
    ...
    function calcCards($cards): float|int
    {
        $noms = array(
            'c2' => 2,
            'c3' => 3,
    ...
    function placeBet($user, $amount): array
    {
    ...
    function insurance($user, $type): void
    {
        switch (intval($type)) {
            case 0:
                $this->data['players'][$user['UID']]['state'] = 1;
    ...
    function command($user, $post): ?array
    {
        switch ($post['command']) {
            case 'new-wallet':
                if (isset($this->data['players'][$user['UID']])) {
    ...
    function createNew(): void
    {
        ...
        if (file_exists('bj_data.json')) {
            $data = json_decode(file_get_contents('bj_data.json'), true)['data'];
