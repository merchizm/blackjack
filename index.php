...
<head>
    <meta charset="UTF-8">
    <title>Rocks | Blackjack</title>
    <link rel="stylesheet" href="assets/styles/bj.css?v=3">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/jquery/2.0.3/jquery.js"></script>
    <script src="assets/scripts/bj.js?v=3"></script>
</head>
<body>
<div class="bj-container">
    <img src="assets/img/table_blackjack.png" class="table">
    ...
    foreach ($game->data['cards'] as $card):
        if ($i == 0 && !$finish_game):
            echo '<div id="dealer-first" class="card card-d bc" style="top:158px;left:484px;"></div>';
        else:
            echo '<div '.($i == 0 ? 'id="dealer-first" ' : '').' class="card card-d '.$card.'" style="top:158px;left:'.(484 + $i * 13).'px;"></div>';
        endif;
        $i++;
    endforeach;
    ...
    foreach ($game->data['players'] as $player):
        ...
        foreach ($player['cards'] as $card):
            echo '<div class="card-'.$player['pos'].' card-nonsplit-'.$player['pos'].' card '.$card.'" style="top:'.($y_arr[$player['pos'] - 1] + $y * 20).'px;left:'.($x_arr[$player['pos'] - 1] - ($player['split'] ? 15 : 0)).'px;"></div>';
            $y++;
        endforeach;
        if ($y):
            echo '<div class="card-holder card-holder-'.$player['pos'].'-s'.($player['split'] ? -1 : 0).'" style="top:'.$y_arr[$player['pos'] - 1].'px;left:'.($x_arr[$player['pos'] - 1] - ($player['split'] ? 15 : 0)).'px;height:'.(65 + 20 * ($y - 1)).'px;"></div>';
        endif;
        $y = 0;
        if ($player['split']) foreach ($player['split'] as $card):
            echo '<div class="card-'.$player['pos'].' card-split-'.$player['pos'].' card '.$card.'" style="top:'.($y_arr[$player['pos'] - 1] + $y * 20 + 20).'px;left:'.($x_arr[$player['pos'] - 1] + 15).'px;"></div>';
            $y++;
        endforeach;
        if ($y):
            echo '<div class="card-holder card-holder-'.$player['pos'].'-s1" style="top:'.($y_arr[$player['pos'] - 1] + 20).'px;left:'.($x_arr[$player['pos'] - 1] + 15).'px;height:'.(65 + 20 * ($y - 1)).'px;"></div>';
        endif;
    endforeach ?>
    ...
    <button class="button-sit sit-1 set-sit-1">Sit</button>
    <button class="button-sit sit-2 set-sit-2">Sit</button>
    <button class="button-sit sit-3 set-sit-3">Sit</button>
    <button class="button-sit sit-4 set-sit-4">Sit</button>
    <button class="button-sit sit-5 set-sit-5">Sit</button>

    <button class="button-sit sit-1 set-ready-1 set-ready">Ready</button>
    <button class="button-sit sit-2 set-ready-2 set-ready">Ready</button>
    <button class="button-sit sit-3 set-ready-3 set-ready">Ready</button>
    <button class="button-sit sit-4 set-ready-4 set-ready">Ready</button>
    <button class="button-sit sit-5 set-ready-5 set-ready">Ready</button>
</div>
</body>
</html>